...
def convert_observation_timestamps(result: ResponseResult) -> ResponseResult:
    ...
    if not isinstance(observation.get('observed_on'), datetime):
        observation['observed_on'] = try_datetime(
            observation.pop('time_observed_at', None) or observation.get('observed_on')
        )
